...
Additionally, the sample error, defined as the difference between the sample statistic and the population value, is a metric that must be factored in when deciding when to reject, or fail to reject a null hypothesis. This is because differences observed in the extracted samples might be attributable to the sample error and not a true effect of the population. This is exhibited in the failure to replicate experiment results, a core tenet of good research. All the results listed in section \ref{data-analysis} and \ref{hypothesis-test} were computed with the code listed in \ref{appendix:statistical-code}
...
\begin{itemize}
	\item Target Coverage Percentage of $90\%$.
	\item Across all three path planning algorithms/strategies (pso, aco and lawn).
	\item Independently seeding 5 iterations per target number trial.
	\item Across a range of 1 to 19 targets.
\end{itemize}
...
We first convert the csv dataset into hdf5 format for data storage and loading efficiency, and compute our sample means thereafter as shown below:
\begin{Verbatim}[commandchars=\\\{\}]
		       pso      aco     lawn
	TargetNum                           
	1              2.0      2.0      2.0
...
\chapter{Statistical Analysis Code}
\label{appendix:statistical-code}
\begin{Verbatim}[commandchars=\\\{\}]
